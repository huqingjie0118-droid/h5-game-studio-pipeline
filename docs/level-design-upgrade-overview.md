...
- [ ] 若未来做程序化关卡，可在 level-design.md §12 程序化保底规则基础上扩展"程序化生成保底规则"子节
- [ ] 建议在仓库 README 的特性列表补一句关卡设计模块描述（如需）

---

## 第二阶段：模块补全（2026-08-03 下午）

在第一阶段 11 个 § 方法论骨架之上，补上三个"加速器"章节，把模块从**方法论**升级为**方法论 + 模式库 + 示例 + 接口协议**：

### 新增 §12 可复用关卡模式库（Pattern Library）
- 7 个经过验证的 H5 空间模块：教学走廊 / 竞技场 / 追逐段 / Boss 门厅 / 奖励死角 / 枢纽广场 / 回声走廊
- 每个模式含布局要点、常见坑、变体；提供组合示例（线性动作关）
- **程序化生成保底规则**：程序只能决定密度与顺序，不允许决定质量底线——每 N 房间强制手工锚点、输出必须过可读性检查、遭遇密度有上下限、抽检 ≥10%

### 新增 §13 完整示例关卡（Worked Example：灰烬回廊）
- 用"从猎手变猎物"的完整案例把 §1–§11 全部串起来：Intent → Shape → Pacing Chart（0:00–5:30 全表）→ Flow Diagram → Encounter Table（3 遭遇含撤退位/教学点）→ 可读性自查（6 项全勾）→ 环境叙事节拍 → Blockout Spec 节选 → 失败信号
- 定位：**格式范本，不是内容答案**——照节奏填自己的游戏
- 叙事亮点示范：Boss 是开场"跑掉的盾兵"，门厅盾阵独缺一面 → 闭环叙事

### 新增 §14 关卡 × 系统 × 数值联动检查（Cross-Check Interface）
- 6 项对账检查：遭遇强度↔DPS 表、奖励投放↔Sources/Sinks、Boss 战↔数值验证、教学阶梯↔机制解锁、节奏↔Session Loop、难度↔心流曲线
- 含对账模板表 + 反例自查（空间侧与数值侧各自自洽但互相矛盾 = GDD 病）
- Gate 2 质量门新增最后一条：**§14 联动检查全过（对账表无红叉）**

### 同步更新
- SKILL.md Reference 列表描述加入 §12–§14
- level-design.md 铁律段新增"高级用法"说明（模式库起步 / 示例校准格式 / 联动检查收尾）
